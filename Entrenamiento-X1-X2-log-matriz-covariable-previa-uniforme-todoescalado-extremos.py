...
a1=simular_logAR1(1000,0.95,1) # Mas de 3 en desviacion da problemas
pd.DataFrame(a1).describe()
salida = np.random.gamma(shape=10,scale=0.10,size=10000)
...
if generar_datos:
  ...
  y_train_sigma_vector= np.random.gamma(shape=10,scale=0.1,size=K)#np.random.uniform(0,3,size=K)#
  ...
  for itera in range(K):
    ...
    y_train_phi_auxiliar = np.min([y_train_phi_auxiliar, 0.90])  # Restringimos la previa para evitar indefiniciones
    y_train_phi_auxiliar = np.max([y_train_phi_auxiliar, 0.1])  # Restringimos la previa para evitar indefiniciones
    for j in range(J):
      ...
      for sitio in range(nsites):
        ...
        producto_auxiliar= covariables_auxiliar*X1_auxiliar*X2_auxiliar#*X3_auxiliar
        cuantil_auxiliar = np.quantile(producto_auxiliar,0.75)
        X_train_auxiliar[sitio] = np.array([np.max([va,cuantil_auxiliar]) for va in producto_auxiliar])
      # Hacemos reshape
      ...
      for lugares in range(nsites):
        if (np.round(X_train_auxiliar[:, :, lugares][0],3)==0).all() or (X_train_auxiliar[:, :, lugares][0]==np.inf).any() or (X_train_auxiliar[:, :, lugares][0]==-np.inf).any():
          ...
          #print(j)
          #print('Cuidado hay un vector de ceros o infinitos, phi: ' + str(y_train_phi_auxiliar) + ' beta 1: ' + str(y_train_beta1_auxiliar)  + 'sigma: ' + str(y_train_sigma_auxiliar) )
          #print(X_train_auxiliar[:, :, lugares][0])
  ...
  np.save('X_train-K10k-m100-J100-X1X2-log-matriz-covariables-uniforme-todoescalado-extremos.npy', X_train) # save
  np.save('y_train-K10k-m100-J100-X1X2-log-matriz-covariables-uniforme-todoescalado-extremos.npy', y_train) # save
  print('Termina generacion de datos')
else:
  X_train = np.load('X_train-K10k-m100-J100-X1X2-log-matriz-covariables-uniforme-todoescalado-extremos.npy')
  y_train = np.load('y_train-K10k-m100-J100-X1X2-log-matriz-covariables-uniforme-todoescalado-extremos.npy')



print('Tamano del archivo: '+ str(len(y_train)) )
...
###
early_stopping = tf.keras.callbacks.EarlyStopping(monitor='loss', mode='min',min_delta=0.001,patience=2,restore_best_weights=True)
...
#OJOJOJOJO
model = tf.keras.Model(inputs, [output1,output2,output3,output6,output7,output8,output9]) #OJO output5
...
epochs=25,batch_size=128,shuffle=True,use_multiprocessing=True,workers=workers_clus,callbacks=[early_stopping],validation_split=0.1)
end = timer()
tiempo_entrenamiento = end - start

print(tiempo_entrenamiento)
model.save('K10k-m100-J100-lstm-X1X2-log-matriz-covariables-100(tanh)-100(tanh)-100(relu)-batch-128-uniforme-todoescalado-extremos')
...
print(tiempo_entrenamiento)
model.save('K10k-m100-J100-lstm-X1X2-log-matriz-covariables-100(tanh)*5-100(tanh)-100(relu)-batch-128-uniforme-todoescalado-extremos')
print('Se guardo el modelo.')
...
print(tiempo_entrenamiento)
model.save('K10k-m100-J100-lstm-X1X2-log-matriz-covariables-100(tanh)*5-100(tanh)-100(relu)*5-batch-128-uniforme-todoescalado-extremos')
print('Se guardo el modelo.')
