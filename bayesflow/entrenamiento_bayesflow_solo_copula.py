...
prior = Prior(prior_fun=model_prior, param_names=[ r"$\beta_3$", r"$\rho$"])
...
def proceso_para_conv2d(params, m):
    ...
    for tiempo in range(m):
        X_train_auxiliar_para_convolucion.append(X3_auxiliar_completo[tiempo,:].reshape(5,5,1).tolist())
...
# Intento 1
nombre_modelo = 'copula-Conv2d-32_64-LSTM_2_512-DENSE_1_256'
...
# Intento 2
nombre_modelo = 'copula-Conv2d-256-LSTM_2_512-DENSE_1_256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=512, summary_dim=512):
        super().__init__()
        timesteps = 100
...
# Intento 4
nombre_modelo = 'copula-Conv2d-256_256_256-LSTM_2_512-DENSE_1_256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=512, summary_dim=512):
        super().__init__()
        timesteps = 100
