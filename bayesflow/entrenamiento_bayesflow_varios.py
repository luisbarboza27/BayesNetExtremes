...
prior = Prior(prior_fun=model_prior, param_names=[r"$\phi$", r"$\sigma$", r"$\gamma_0$",r"$\gamma_1$",r"$\gamma_2$",r"$\gamma_3$"])
...
def proceso2(params, m):
    y_train_beta1_auxiliar=0.5
    y_train_beta3_auxiliar =  np.random.gamma(shape=1/3,scale=100,size=1)[0]
    y_train_rho_auxiliar =    np.random.uniform(0,2*np.max(dist_mat),size=1)[0]
    y_train_beta3_auxiliar = np.max([y_train_beta3_auxiliar,3])
    y_train_phi_auxiliar,y_train_sigma_auxiliar,g1,g2,g3,g4 = params
    y_train_gamma_auxiliar = [g1,g2,g3,g4]
    X_train_auxiliar = np.zeros((nsites+2, m))
    ...
    for sitio in range(nsites):
     ...
     # Hacemos reshape
    X_train_auxiliar[nsites-1+1]=np.repeat(y_train_beta3_auxiliar,m)
    X_train_auxiliar[nsites-1+2]=np.repeat(y_train_rho_auxiliar,m)
    X_train_auxiliar = X_train_auxiliar.reshape(1, nsites+2, m).transpose(0, 2, 1)
    return(np.array([X_train_auxiliar]))
time_points=100


simulator2 = Simulator(simulator_fun=partial(proceso2, m=time_points))
...
def configurator(forward_dict):
    ...
    params[:,0]=np.log(params[:,0]/(2-params[:,0]))
    params[:,1]=np.log(params[:,1]/(4-params[:,1]))
...
nombre_modelo='LSTM-2-512_CONV1D-256-5_DENSE-1-256'
class CustomLSTM(tf.keras.Model):
    ...
    def call(self, x, **kwargs):
        x = tf.reshape(x, (-1, 100, 20+2))  # Ajusta según sea necesario
        out = self.LSTM(x)
        return out
...
inference_net = InvertibleNetwork(num_params=6, num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,   coupling_design = 'spline')
...
nombre_modelo='LSTM-2-1000_CONV1D-256-5_DENSE-1-256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=1000, summary_dim=512):
        super().__init__()
        timesteps = 100
        features = 20+2
        self.LSTM = tf.keras.Sequential(
...
nombre_modelo='LSTM-2-1000_CONV1D-256-5_DENSE-1-1000'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=1000, summary_dim=1000):
        super().__init__()
        timesteps = 100
        features = 20+2
        self.LSTM = tf.keras.Sequential(
...
nombre_modelo='LSTM-2-1000_DENSE-1-1000'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=1000, summary_dim=1000):
        super().__init__()
        timesteps = 100
...
nombre_modelo='LSTM-2-128_CONV1D-256-5_DENSE-4-256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=128, summary_dim=512):
        super().__init__()
        timesteps = 100
        features = 20+2
        self.LSTM = tf.keras.Sequential(
...
try:
    nombre_modelo='LSTM-2-128_CONV1D-256-5_DENSE-4-256_escalado'
    class CustomLSTM(tf.keras.Model):
        def __init__(self, hidden_size=128, summary_dim=512):
            super().__init__()
            timesteps = 100
    ...
    inference_net = InvertibleNetwork(num_params=6, num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,   coupling_design = 'spline')
    amortizer = AmortizedPosterior(inference_net, summary_net, name=nombre_modelo)
    trainer = Trainer(amortizer=amortizer, generative_model=model, memory=True, checkpoint_path = nombre_modelo,configurator=configurator)
    history = trainer.train_online(epochs=n_epochs, iterations_per_epoch=n_iterations_per_epoch, batch_size=n_batch_size,early_stopping=True,validation_sims=1000)
    valid_sim_data_raw = model_escalado(batch_size=500)
    valid_sim_data = trainer.configurator(valid_sim_data_raw)
    posterior_samples = amortizer.sample(valid_sim_data, n_samples=100)
    fig = diag.plot_recovery(posterior_samples, valid_sim_data["parameters"], param_names=prior.param_names)
    fig.savefig(nombre_modelo + '.PNG')
    ...
    print('Fallo el escalado')



############################################
try:
    nombre_modelo='LSTM-2-128_CONV1D-256-5_DENSE-4-256_permutation_None'
    class CustomLSTM(tf.keras.Model):
        def __init__(self, hidden_size=128, summary_dim=512):
            super().__init__()
            timesteps = 100
    ...
    inference_net = InvertibleNetwork(num_params=6, num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,   coupling_design = 'spline',permutation=None)
    amortizer = AmortizedPosterior(inference_net, summary_net, name=nombre_modelo)
    trainer = Trainer(amortizer=amortizer, generative_model=model, memory=True, checkpoint_path = nombre_modelo)
    history = trainer.train_online(epochs=n_epochs, iterations_per_epoch=n_iterations_per_epoch, batch_size=n_batch_size,early_stopping=True,validation_sims=1000)
    valid_sim_data_raw = model(batch_size=500)
    ...
    print('No se pudo hacer el sin permutation')
...
nombre_modelo='LSTM-2-512_CONV1D-256-5_DENSE-4-256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=512, summary_dim=512):
        super().__init__()
        timesteps = 100
...
nombre_modelo='LSTM-2-512_CONV1D-256-5_DENSE-10-256'
class CustomLSTM(tf.keras.Model):
    def __init__(self, hidden_size=512, summary_dim=512):
        super().__init__()
        timesteps = 100
