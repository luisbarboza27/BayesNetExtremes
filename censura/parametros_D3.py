...
parametros_D3= [ r"$\phi$", r"$\sigma$",r"$\beta_3$", r"$\rho$"]
prior_D3 = Prior(prior_fun=model_prior_D3, param_names=parametros_D3)
prior_means_D3, prior_stds_D3 = prior_D3.estimate_means_and_stds()
 

##################


def proceso_D3(params, m):
...
time_points=m
simulator_D3 = Simulator(simulator_fun=partial(proceso_D3, m=time_points))
model_D3 = GenerativeModel(prior_D3, simulator_D3, name="simulador_proceso")
data_D3 = model_D3(batch_size=1)


nombre_modelo = 'parametros_D3'
from tensorflow.keras.layers import ConvLSTM2D, BatchNormalization, Conv2D, MaxPooling2D, TimeDistributed, Flatten, Dense
class CustomLSTM_D3(tf.keras.Model):
...
simul_previa_D3 = model_D3(batch_size=n_iterations_per_epoch*n_batch_size)
# try:
...
inference_net_D3 = InvertibleNetwork(num_params=4 ,num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,   coupling_design = 'spline')
amortizer_D3 = AmortizedPosterior(inference_net_D3, summary_net_D3, name=nombre_modelo)
trainer_D3 = Trainer(amortizer=amortizer_D3, generative_model=model_D3, memory=True, checkpoint_path = nombre_modelo)
history_D3 = trainer_D3.train_offline(simulations_dict=simul_previa_D3,epochs=n_epochs,batch_size=n_batch_size,early_stopping=True,validation_sims=128)
valid_sim_data_raw_D3 = model_D3(batch_size=128)
valid_sim_data_D3 = trainer_D3.configurator(valid_sim_data_raw_D3)
posterior_samples_D3 = amortizer_D3.sample(valid_sim_data_D3, n_samples=100)
fig = diag.plot_recovery(posterior_samples_D3, valid_sim_data_D3["parameters"], param_names=parametros_D3)
print('######################################################################')
