...
inference_net_DY = InvertibleNetwork(num_params=4 ,num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,   coupling_design = 'spline')
amortizer_DY = AmortizedPosterior(inference_net_DY, summary_net_DY, name=nombre_modelo)
trainer_DY = Trainer(amortizer=amortizer_DY, generative_model=model_DY, memory=True, checkpoint_path = nombre_modelo)
...
valid_sim_data_raw_DY = model_DY(batch_size=128)
...
fig = diag.plot_recovery(posterior_samples_DY, valid_sim_data_DY["parameters"], param_names=parametros_DY)
