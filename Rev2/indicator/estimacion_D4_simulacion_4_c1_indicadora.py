...
#3
nombre_modelo='covariables_D4_simulacion_extremo_V4'+'2_15'+'_indicadora'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D4',cov,parametros,n_epochs,n_batch_size,1024, 1024,nombre_modelo)
###################################################################
###################################################################
###################################################################
cov = cov_original
...
prior_D4 = Prior(prior_fun=model_prior_D4, param_names=parametros)
...
def proceso_D4(params, m):
    ...
    Indicator = Indicator.reshape(1, nsites, m).transpose(0, 2, 1)
    ...
    for tiempo in range(m):
        ...
        matriz_ind = Indicator[0][tiempo,:].reshape(10,10)
        #Indicator_para_convolucion.append(matriz_ind.reshape(10,10,1).tolist())
...
def funcion_prediccion_parametros(simul_previa_D4,cov,parametros,n_epochs,n_batch_size,summary_net_D4,nombre_modelo):
    ...
    inference_net_D4 = InvertibleNetwork(num_params=len(parametros), num_coupling_layers=10, coupling_settings=COUPLING_NET_SETTINGS,coupling_design='spline')
    amortizer_D4 = AmortizedPosterior(inference_net_D4, summary_net_D4, name=nombre_modelo)
    trainer_D4 = Trainer(amortizer=amortizer_D4, generative_model=model_D4, memory=False, checkpoint_path = nombre_modelo)
    valid_sim_data_raw_D4 = model_D4(batch_size=1)
    #valid_sim_data_D4 = trainer_D4.configurator(valid_sim_data_raw_D4)
...
class CustomLSTM_D4(tf.keras.Model):
    def __init__(self, hidden_size=1000, summary_dim=2000):
        ...
        timesteps = m
        self.LSTM = tf.keras.Sequential(
            [   tf.keras.layers.Input((timesteps,10, 10, 2)),
...
#6
nombre_modelo='parametros_D4_simulacion_extremo_V14'+'2_15'+'_indicadora'
summary_net_D4 = CustomLSTM_D4(1024, 1024)
amortizer_parametros,trainer_parametros,valid_sim_data_2=funcion_prediccion_parametros('simul_previa_D4',cov,parametros,n_epochs,n_batch_size,summary_net_D4,nombre_modelo)

def proceso_D4_estimacion(params_covariables,params_parametros, nsites, m,cov,dist_mat):
...
def datos_para_red1(parametros,nsites,m,simulacion,cov_matriz):
    ...
    auxiliar = simulacion.copy()
    auxiliar[0][0][:,2*nsites]=y_train_phi_auxiliar
    auxiliar[0][0][:,2*nsites+1]=y_train_sigma_auxiliar
    auxiliar[0][0][:,2*nsites+2]=y_train_beta3_auxiliar
    auxiliar[0][0][:,2*nsites+3]=y_train_rho_auxiliar
    return(auxiliar)
...
simulacion_proceso = proceso_D4_estimacion(gammas_real,param_real, nsites, m,cov,dist_mat)
...
print(df_result)
#7
df_result.to_csv('estimacion_r_simulacion_D4_extremo_V4_V14_c1_indicadora.csv')
df_simul.to_csv('estimacion_t_simulacion_D4_extremo_V4_V14_c1_indicadora.csv')
