...
covariables_guanacaste <- read.csv(
  "covariables_guanacaste_train_test.csv"
)
...
# ============================================================
# 4. SEPARAR TRAIN, TEST Y PADDING
# ============================================================
...
print(paste("TRAIN:", nrow(loc_train)))
print(paste("TEST:", nrow(loc_test)))
print(paste("PADDING:", nrow(loc_padding)))

# Número de sitios
nsites <- nrow(loc_train)

locaciones_test <- loc_test
...
locaciones_train_test <- rbind(loc_train,loc_test)
...
Z1 <- loc_train$lon  # primera covariable espacial
Z2 <- loc_train$lat  # segunda covariable espacial
Z12 <- loc_train$lon^2  # primera covariable espacial al cuadrado
Z22 <- loc_train$lat^2  # segunda covariable espacial al cuadrado
Z3 <- loc_train$alt  # tercera covariable (altitud)
Z32 <- loc_train$alt^2  # altitud al cuadrado
...
dist_mat <- as.matrix(dist(loc_train[, c("lon", "lat")]))  # matriz de distancia (nsites x nsites)
...
m <- nrow(datos_guanacaste) / nrow(locaciones_train_test)
...
datos_guanacaste_filtrados <- merge(datos_guanacaste, loc_train, by = c("lon", "lat"))
...
for (sitio in 1:nsites) {
  lon_actual <- loc_train$lon[sitio]
  lat_actual <- loc_train$lat[sitio]
...
##################### Para cálculo de errores cuantilicos
cuantiles_train_test <- matrix(0, nrow = length(76:99), ncol = nsites_train_test)

for (j in 1:nsites_train_test) {
  cuantiles_train_test[, j] <- sapply(76:99, function(k) quantile(X_train_guanacaste_train_test[, j], k / 100,na.rm = T))
}

df_errores <- data.frame(matrix(ncol = 2 + nsites_train_test, nrow = 0))
x <- c("modelo", "error", paste0('s', 1:nsites_train_test))
colnames(df_errores) <- x
...
library(dplyr)
library(tidyr)
combinations <- expand.grid(D =  c("7","Y"), M = 2:7)
# Crear carpeta de salida si no existe
dir.create("calculo_WIS", showWarnings = FALSE)



# Creamos dataframe de percentiles observados:

df_X_train_guanacaste_train_test_percentil <- df_X_train_guanacaste_train_test %>% 
  group_by(variable) %>% 
  reframe(
    aq = round(seq(0.75, 0.99, by = 0.01), 2),
    q = quantile(valor, probs = aq, na.rm = TRUE)
  ) %>% 
  ungroup()
...
clusterExport(cl, varlist = c(
  ...
  "m",
  "dist_mat_train_test",
  "df_X_train_guanacaste_train_test_percentil",
  "proceso_estimacion",
  "simular_logAR1",
  "simular_X1",
  "simular_X3",
...
procesar_modelo <- function(i) {
  ...
  log_file <- paste0("log_", modelo_completo,'-train-quantile', ".txt")
  log_msg <- function(txt) {
    cat(paste0(Sys.time(), " | ", txt, "\n"),
        file = log_file, append = TRUE)
  }
  archivo <- paste0('WIS_phi_train_quantile', modelo_completo, '.RData')
  ruta <- file.path("calculo_WIS", archivo)
  
  archivo_error <- paste0('errores_train_quantile', modelo_completo, '.RData')
  ruta_error <- file.path("calculo_WIS", archivo_error)
  ...
  tryCatch({
    ...
    df_q <- data.frame()
    error_absoluto_muestras <- matrix(ncol = nsites_train_test)
    error_cuadratico_muestras <- matrix(ncol = nsites_train_test)
    error_absoluto_muestras_med <- matrix(ncol = nsites_train_test)
    error_cuadratico_muestras_med <- matrix(ncol = nsites_train_test)
    df_errores_aux <- data.frame(modelo = rep(modelo_completo, 4), error = c('abs_mean', 'sqr_mean','abs_median', 'sqr_median'))
    ...
    for (muestra in 1:cantidad_simulaciones) {
      ...
      error_absoluto_med <- c()
      error_cuadratico_med <- c()
      
      
      # Calculo de percentiles
      grilla <- round(seq(0.75,0.99,0.01),2)
      
      for (sitio in 1:ncol(simul)) {
        cuantiles_simul_sitio <- sapply(grilla, function(k) quantile(simul[, sitio], k,na.rm = T))
        # Para IS:
        df_q_aux <- data.frame(muestra=rep(muestra,length(grilla)),aq=grilla,variable=rep(sitio,length(grilla)),valor=cuantiles_simul_sitio)
        df_q <- rbind(df_q,df_q_aux)
        
        
        # Para errores:
        cuantiles_train_test_sitio <- cuantiles_train_test[, sitio]
        dif <- cuantiles_simul_sitio - cuantiles_train_test_sitio
        
        error_absoluto <- c(error_absoluto, mean(abs(dif),na.rm = T))
        error_cuadratico <- c(error_cuadratico, mean(dif^2,na.rm = T))
        error_absoluto_med <- c(error_absoluto_med, median(abs(dif),na.rm = T))
        error_cuadratico_med <- c(error_cuadratico_med, median((dif)^2,na.rm = T))
        
      }
      
      error_absoluto_muestras <- rbind(error_absoluto_muestras, error_absoluto)
      error_cuadratico_muestras <- rbind(error_cuadratico_muestras, error_cuadratico)
      error_absoluto_muestras_med <- rbind(error_absoluto_muestras_med, error_absoluto_med)
      error_cuadratico_muestras_med <- rbind(error_cuadratico_muestras_med, error_cuadratico_med)
      
      
    }
    log_msg("Concluye simulaciones y calculo de error.")
    
    df_errores_aux[1:4, paste0('s', 1:nsites_train_test)] <- matrix(c(error_absoluto_muestras %>% 
                                                                        colMeans(na.rm = TRUE), error_cuadratico_muestras %>% 
                                                                        colMeans(na.rm = TRUE),
                                                                      colMedians(error_absoluto_muestras_med,na.rm = TRUE),
                                                                      colMedians(error_cuadratico_muestras_med,na.rm = TRUE)
                                                                      
    ), nrow = 4, ncol = nsites_train_test, byrow = TRUE)


    
    log_msg("Inicia percentiles")
    
    df_q_upper_lower <- df_q %>% 
      group_by(aq, variable) %>% 
      reframe(
        percentil = round(seq(0.05, 0.95, by = 0.05), 2),
        q = quantile(valor, probs = percentil, na.rm = TRUE)
      ) %>% 
      ungroup()
    ...
    tv <- unique(df_q_upper_lower$aq)
    variablev <- unique(df_q_upper_lower$variable)
    ...
    for (tt in tv) {
      for (vv in variablev) {
        for (upper in vector_upper) {
          ...
          lower_upper_t <- df_q_upper_lower %>% 
            filter(aq==tt,variable==vv, percentil %in% c(upper, lower)) %>% 
            arrange(percentil)
          
          mediana_t <- df_q_upper_lower %>% 
            filter(aq==tt,variable==vv, percentil==0.5)
          
          q_aq <- df_X_train_guanacaste_train_test_percentil %>% 
            filter(aq==tt,variable==paste0('X',vv))
          
          IS <- lower_upper_t$q[2]-lower_upper_t$q[1] +
            2/lower*ifelse(lower_upper_t$q[1]>q_aq$q[1],
                           lower_upper_t$q[1]-q_aq$q[1],0) +
            2/lower*ifelse(lower_upper_t$q[2]<q_aq$q[1],
                           q_aq$q[1]-lower_upper_t$q[2],0)
    ...
    save(df_errores_aux, file = ruta_error)
