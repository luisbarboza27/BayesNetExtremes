...
def datos_para_red1(parametros,nsites,m,simulacion,cov_matriz):
    y_train_phi_auxiliar,y_train_sigma_auxiliar = parametros
...
def datos_para_reD1(covariables,nsite_completados,m,simulacion,cov_matriz):
...
def gibbs(n_iter,nombres_parametros,n_covariables,parametros_input,nsites,nsites_completados,m,simulacion_proceso,cov,valid_sim_data_1,trainer_covariables,amortizer_covariables,valid_sim_data_raw_DY,trainer,amortizer):
    ...
    try:
        ...
        for k in range(n_iter):
            ...
            input_red_2 = datos_para_reD1(posterior_samples_covariables,nsites_completados,m,simulacion_proceso,cov)
            ...
            # Cambio
            p1,p2= amortizer.sample(valid_sim_data_raw_DY_c, n_samples=1)[0]
        ...
        fin = datetime.now()
        duracion = fin - inicio
        df_simul = pd.DataFrame({nombres_parametros[0]:p1_pred,nombres_parametros[1]:p2_pred})
...
def model_prior():
    ...
    y_train_auxiliar = [y_train_phi_auxiliar,y_train_sigma_auxiliar]
   
    previas = np.array(y_train_auxiliar)
    return(previas)
 
 
parametros= [ r"$\phi$", r"$\sigma$"]
...
def proceso(params, m):
    y_train_phi_auxiliar,y_train_sigma_auxiliar = params
    X_train_auxiliar = np.zeros((nsite_completados, m)) # 2
    Indicator = np.zeros((nsite_completados, m))
...
model_name = 'parametros_D1_3_padding_sim_apli'
...
parametros_covariables = [r"$\gamma_{0}$",r"$\gamma_{{lon}}$",r"$\gamma_{{lat}}$",r"$\gamma_{{alt}}$",r"$\gamma_{{lon}^2}$",r"$\gamma_{{lat}^2}$",r"$\gamma_{{alt}^2}$"]
...
class CustomLSTM_covariables(tf.keras.Model):
 ...
 #               tf.keras.layers.Flatten(),
    ...
    def call(self, x, **kwargs):
        x = tf.reshape(x, (-1, time_points,  nsites+2))  # Ajusta según sea necesario
        out = self.LSTM(x)
        return out
...
######################################################################################################
nombre_modelo='covariables_D1_aplicacion_M7_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
trace=gibbs(n_posteriores,nombres_parametros,7,parametros_input,nsites,nsite_completados,m,simulacion_proceso,cov,valid_sim_data_1,trainer_covariables,amortizer_covariables,valid_sim_data_raw_parametros,trainer,amortizer)
...
nombre_modelo='covariables_D1_aplicacion_M6_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
...
cov = cov_original[:,0:covariable_quitar]
parametros=parametros[0:covariable_quitar]
nombre_modelo='covariables_D1_aplicacion_M5_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
...
cov = cov_original[:,0:covariable_quitar]
parametros=parametros[0:covariable_quitar]
nombre_modelo='covariables_D1_aplicacion_M4_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
...
cov = cov_original[:,0:covariable_quitar]
parametros=parametros[0:covariable_quitar]
nombre_modelo='covariables_D1_aplicacion_M3_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
...
cov = cov_original[:,0:covariable_quitar]
parametros=parametros[0:covariable_quitar]
nombre_modelo='covariables_D1_aplicacion_M2_V2'
amortizer_covariables,trainer_covariables,valid_sim_data_1=funcion_prediccion_covariables('simul_previa_D1',cov,parametros,n_epochs,n_batch_size,1024, 128,nombre_modelo)
