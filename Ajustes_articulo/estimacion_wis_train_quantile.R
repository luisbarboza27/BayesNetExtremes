...
combinations <- expand.grid(D =  c("1","2","3","8","Y"), M = 1:7)
  ...
  tryCatch({
    ...
    for (muestra in 1:cantidad_simulaciones) {
      ...
      for (sitio in 1:ncol(simul)) {
        for (perc in grilla) {
          perc_sitio <- quantile(simul[,sitio],perc)
          df_q_aux <- data.frame(muestra=muestra,aq=perc,variable=sitio,valor=perc_sitio)
